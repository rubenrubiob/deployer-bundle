...
git strategy. It provides commands to do it easily.
...
By default, the bundle only checkouts a specific version (either a tag or no)
and clears the cache, but there are optional tasks that may also be executed.
...
See the [commands](#commands) reference for deploying. 
...
It will deploy to the last tag created or, if set, to a specific tag version. That is the only difference
with the [deploy](#command) options.
...
```
Usage:
  rrb:deployer:tag [options] [--] [<hosts>]...

Arguments:
  hosts                     Host(s) name(s) defined in config.yml to deploy to (separate multiple hosts with a space). If omitted, first host defined will be used.

Options:
  -t, --tag=TAG             Tag number to checkout, if any.
      --all                 If set, it will deploy to all servers defined in config.yml. It overrides hosts argument.
      --composer-update     If set, it will update composer. It overrides value in config.yml.
      --assets-install      If set, it will install assets. It overrides value in config.yml.
      --database-migration  If set, it will execute migrations of Doctrine. It overrides value in config.yml.
...
Help:
  Command to checkout a tag using git
```