...
use Rrb\DeployerBundle\Command\Deploy\TagCommand;
...
class TagCommandTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCommandArguments()
    {
        ...
        $this->assertEquals(
            sprintf('%s %s %s ', $service->tag('host1'), $service->tag('host2'), $service->tag('host3')),
        ...
        $this->assertEquals(
            sprintf('%s %s ', $service->tag('host2'), $service->tag('host3')),
            $this->commandTester->getDisplay()
        );
        ...
        $this->assertEquals(
            sprintf('%s ', $service->tag('host1')),
            $this->commandTester->getDisplay()
        );
    }

    /**
     * Test options for TagCommand
     */
    public function testCommandOptions()
    {
        ...
        $this->assertEquals(
            sprintf('%s ', $service->tag('host1', $options)),
            $this->commandTester->getDisplay()
        );
    }
    ...
    private function createApplication()
    {
        ...
        $this->command = $application->find('rrb:deployer:tag');
